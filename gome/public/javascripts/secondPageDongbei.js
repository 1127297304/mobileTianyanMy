;(function(){
	...
	function askForData(){
		$.ajax({
			...
			success: function(data){
				...
				//渲染默认显示的图表
				var initShow = globalData.areaDataValue[0];//获取默认显示的大区
	...
	function createTabControl(data){
		//假定返回的按大小排列好的 分区数组
		//console.log(data);
		var arr = data.areaNameRankin;console.log(arr)
		var arrMoney = data.areaLoanMoneyRanking;
		var target = document.querySelector('#tab_control_con');
		var dataValue = data.areaDataValue;console.log(dataValue)
		//var newDiv = document.create
		//for(var i=0,len=arr.length;i < len ; i++){
			target.appendChild(createLi(arr[0],arrMoney[0],dataValue[0]));
		//}
